...
15. **시도 15 (마스킹 버그 및 물리 엔진 모순 발견):**
    - **날짜:** 2026-06-02
    - **내용:** 시도 14의 모델(Time-Conditioned MLP)을 그대로 사용하면서 마스킹 로직을 적용함. 하지만 사용자의 당초 의도("공이 바닥에 닿으면 시뮬레이션을 끝내고 학습 안 함")와 달리, 정답 데이터를 생성하는 물리 엔진(`app.py`)은 공이 바닥에 닿은 후 튕겨 오르게(바운스) 구현되어 있었음. 반면 학습 코드는 바운스 첫 프레임 이후를 무작정 마스킹(`cumsum` 버그로 인해 두 번째 포물선을 마스킹하지 못하고 엉뚱하게 오작동함)하는 상태로 학습이 진행됨.
    - **결과:** **시각화 화면에서 궤적이 미친 듯이 튀는(Jittering/Jumping) 심각한 현상 발생.** 정답은 튕기는데 AI는 그 뒷부분을 제대로 학습하지 못하고, 게다가 억지로 바운스를 흉내 내려다 고주파 푸리에 변환 탓에 값이 요동침. 결국 `Z <= -0.75` 패딩 자르기 로직조차 빗나가면서 화면 내내 궤적이 이리저리 튀는 엉망진창 상태가 됨. 이를 해결하기 위해 물리 엔진 자체를 '바닥 충돌 시 패딩(종료)'으로 수정하고 데이터를 다시 뽑기로 결론 내림. (LSTM 등 아키텍처 개편도 논의 중)
