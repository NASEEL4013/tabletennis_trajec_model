...
7. **시도 7 (Physics-Informed Cosine Similarity Loss):**
   - **날짜:** 2026-05-29
   - **내용:** 사용자의 아이디어를 채택하여, 속도 벡터의 진행 방향(각도) 일치 여부를 검사하는 코사인 유사도(Cosine Similarity) 방향 Loss를 투입. (단, 가속도 및 속도의 크기(Magnitude) 제약은 실수로 배제됨)
...
8. **시도 8 (Physics-Informed Kinematic Derivative Loss):**
   ...
   - **결과:** **대성공!** Train MAE 0.008(0.8cm), Test MAE 0.014 달성. 뱀처럼 구불거리던 현상이 완전히 사라지고, 코사인 Loss의 얍삽한 꼼수도 차단됨. 모델이 드디어 '물리 법칙'을 완벽하게 모방하여 바운드 지점에서 뾰족한 V자에 가까운 형태를 생성해 냈음. 원샷 예측(MLP)의 한계를 물리 기반 제약(PINN)으로 돌파한 가장 성공적인 시도!
