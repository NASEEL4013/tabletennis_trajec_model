...
## 🎯 최종 목표 (연구의 본질)
복잡하고 무거운 수학적 물리 연산(SciPy `solve_ivp`, RK45)을 매 런타임마다 수행하는 대신, 물리 법칙의 결과를 학습한 **인공지능 모델**이 빠른 추론(Inference)을 통해 500스텝의 3D 궤적을 즉시 렌더링하도록 구현함.

**핵심 철학: Physics as a Neural Network**
- **데이터 생성:** 공기저항, 마그누스 효과, 중력, 탁구대 경계, 네트 충돌(Soft-wall), 바닥 추락 등의 물리 법칙이 적용된 정밀한 수학적 시뮬레이터로 300만 개의 합성 데이터를 대량 생성함.
- **다차원 학습:** 생성된 방대한 8차원 공간의 파라미터(입력)와 500스텝 궤적(출력)의 관계를 AI가 근사(Approximation)하도록 학습시킴.
...
- **데이터 추출:** 무작위로 샘플링된 300만 개의 8D 입력을 물리 엔진에 인가하여 정답지(1500D) 세트를 추출함. (`dataset_random.npz`로 저장)

### Step 2. AI 물리 모델 아키텍처 진화 (Evolution of Physics Decoder)
- **파일:** `train_baseline.py`
- **현재 구조 (Expanding MLP):**
  - 차원이 점진적으로 확장되는 순수 확장형 구조 (`8 -> 256 -> 512 -> 1024 -> 1500`) 도입.
  - 복잡한 기법(ResNet 등)을 배제하고 가장 단순한 형태의 다층 퍼셉트론(MLP)으로 회귀함.
- **손실 함수 (Loss):** 기본 L2 Loss (MSE Loss) 적용.
- **데이터셋 처리:** 300만 개 데이터에 대해 Z-score 정규화 수행 (`mlp_norm_standard_random.npy`).
...
### 최적화 가이드 (필수 준수 사항)
1. **Batch Size 극대화:** GPU VRAM이 8GB로 제한적이나, 현재 다층 퍼셉트론(MLP) 모델의 파라미터가 매우 가벼우므로 GPU 병렬 처리를 위해 `BATCH_SIZE = 4096` 이상을 적극 권장함. (에포크 훈련 속도 기하급수적 향상)
2. **데이터 로딩 전략:** 시스템 RAM(128GB)의 여유를 활용하여 300만 개 이상의 데이터(약 18GB)를 디스크 I/O 병목 없이 메모리에 한 번에 적재(`TensorDataset`)하여 사용함.
...
5. **시도 5 (High-Density Data + ResNet + L1 Loss):**
   - **날짜:** 2026-05-29
   - **내용:** 파라미터 범위를 축소하여 데이터 밀도를 대폭 높인 300만 개 데이터셋 적용 + Expanding MLP + ResBlock + L1 Loss (GPU 최적화를 위해 BATCH_SIZE를 4096으로 상향).
   - **데이터 증강 및 파라미터 범위 변경 사항:**
     | 파라미터 | 이전 범위 (200만 개) | 변경된 범위 (300만 개) |
     |---|---|---|
...
8. **시도 8 (Physics-Informed Kinematic Derivative Loss):**
   ...
   ![시도 8 완 완벽한 물리 법칙 모방](results/attempt8_kinematic.png)
...
16. **시도 16 (완벽한 마스킹, SiLU, 최적화 모델 스케일링):**
    ...
    - **결과:** **목표 달성 (Test MAE 0.0067 / Train MAE 0.0032)!** 바운스 이후의 노이즈 데이터를 완벽히 차단(Masking)하여 약 3~6mm의 초정밀 오차율을 기록함. 탁구대 반사 및 바닥 충돌이 물리 엔진과 99% 유사하게 렌더링됨. 딥러닝의 연속 함수 특성상 바운드 순간의 V자가 미세하게 둥글어지는 1%의 스무딩 현상은 잔존하나 시뮬레이션 용도로 적합함.
...
17. **시도 17 (가우시안 정규 분포 기반 데이터셋 밀도 최적화):**
    ...
    - **목적:** 정상적인 랠리 궤적의 비율을 높이기 위해 각 8D 파라미터의 평균을 일반적인 랠리 조건에 맞추고 표준편차를 조절하여, "정상 랠리 70% + 이상치(Edge case) 30%" 비율의 신규 데이터셋(`dataset_gaussian_mixed.npz`) 300만 개 생성.
...
19. **시도 19 (나이퀴스트 한계 주파수 보장 & 순수 L1 Loss 롤백):**
    - **날짜:** 2026-06-11
    - **내용:** 스무딩의 원인인 `SmoothL1Loss`를 제거하고 `순수 L1Loss`로 복귀. 푸리에 위치 인코딩의 주파수 대역(`num_freqs=10`)이 나이퀴스트 한계에 부합함을 확인하고 훈련 진행.
