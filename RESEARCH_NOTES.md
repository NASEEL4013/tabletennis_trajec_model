...
2. **시도 2 (Kinematic Loss 도입):**
   ...
   - **목적:** 시도 1에서 발생한 치명적인 '파도타기(구불거림)' 현상을 잡기 위해, 위치 오차(MSE)뿐만 아니라 **속도(Velocity)와 가속도(Acceleration)의 오차에도 페널티(L1 Loss)**를 부여하여 물리적인 관성과 부드러움을 강제함. (기울기 폭발을 막기 위해 가속도 계산 시 L2가 아닌 L1 Loss를 채택함)
   - **상태:** 학습 대기 중
